...
export class CreateMissionUseCase {
  ...
  async execute(dto: CreateMissionDto) {
    ...
    // Validation
    const drone = await this.dronesRepository.findOne(dto.droneId, fleetOwnerId);
    if (!drone) {
      throw new NotFoundException(`Drone ${dto.droneId} was not found.`);
    }

    if (drone.status !== DroneStatus.AVAILABLE) {
      throw new BadRequestException(`Drone ${dto.droneId} is not available for a new mission.`);
    }
    ...
    const overlap = await this.missionsRepository.findOverlapping(dto.droneId, plannedStart, plannedEnd);
    if (overlap) {
      void this.notificationsService
        ...
        .catch(() => undefined);
      throw new ConflictException('This drone already has a mission scheduled during this time window.');
    }
